...
#### Adam (lo veremos en Aprendizaje Profundo)


---

![:scale 100%](./img/contours_evaluation_optimizers.gif)
...
* Reducimos número de parámetros de $\mathcal{O}(p^2)$ a $\mathcal{O}(kp)$.
...
* Podemos ver que es la frontera de decisión la conforma una combinación lineal de funciones de cada variable predictora
\begin{eqnarray*}
\log \frac{P(Y=Y_k \vert x)}{P(Y=Y_j \vert x)} &=& \log \frac{f_k(x)}{f_j(x)} + \log \frac{\pi_k(x)}{\pi_j(x)} \\
&=& \log \frac{\pi_k(x)}{\pi_j(x)} + \sum_{i=1}^p \log \frac{f_{ki}(x_i)}{f_{ji}(x_i)} \\
&=& \log \frac{\pi_k(x)}{\pi_j(x)} + \sum_{i=1}^p g_{lji}(x_i)
\end{eqnarray*}
...
* **Ejercicio**: probar que a medida que el número de observaciones de entrenamiento $N \rightarrow \infty$, 1-NN tiene una tasa de error que no será peor que el doble la tasa de error de Bayes.
