...
* Asumimos que nuestra distribución inicial para $z_1$ es uniforme.
...
B <- matrix(data =  , nrow = 6, ncol = 2, dimnames = list(seq(1,6), c("I", "S")))

# Definimos el prior sobre el primer estado.
pi <- matrix(data = log(c( , )), nrow = 2, ncol = 1, dimnames = list(c("I", "S"), ""))
...
donde  $V_{t,k} = \max_{z_1, \ldots, z_{t-1}} p(x_1, \ldots, x_t, z_1, \ldots, z_t)$, con $z_t = k$., con $z_t = k$.
...
for (t in 2:length(observaciones)) {
  ...
  # calcula aqui el caso recursivo
  V[t, ] <- 
  # anota aqui el valor que haya alcanzado el maximo en la linea anterior
  Ptr[t, ] <-
